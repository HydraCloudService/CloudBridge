...
namespace hydracloud\cloud\bridge\module\sign\listener;

use hydracloud\cloud\bridge\module\sign\CloudSign;
...
final class SignListener implements Listener {
    ...
    public function onChange(SignChangeEvent $event): void {
        if ($event->getNewText()->getLine(0) == "[HydraCloud]") {
            if ($event->getPlayer()->hasPermission("hydracloud.cloudsign.add")) {
        ...
        if (($sign = CloudSignModule::get()->getCloudSign($event->getBlock()->getPosition())) !== null) {
            ...
            if (Server::getInstance()->getTick() >= CloudBridge::getInstance()->signDelay[$event->getPlayer()->getName()]) {
                ...
                if ($sign->hasUsingServer() && !$sign->getUsingServer()->getTemplate()->isMaintenance()) {
                    if (CloudAPI::servers()->current() != null && CloudAPI::servers()->current()?->getName() == $sign->getUsingServer()->getName()) {
    ...
    public function onBreak(BlockBreakEvent $event): void {
        if (($sign = CloudSignModule::get()->getCloudSign($event->getBlock()->getPosition())) !== null) {
            if ($event->getPlayer()->hasPermission("hydracloud.cloudsign.remove")) {
